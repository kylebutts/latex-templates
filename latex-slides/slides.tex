...
\begin{frame}{Components}{Example of tikz figure}
...
\begin{tikzpicture}[node distance=1.5cm]
    ...
    \node[below right of = d, text centered] (x) [draw, shape=circle, line width = 1] {$X$};
    \node[above right of = x, text centered] (y) {$Y$};
    \node[below of = x, text centered, purple] (z) {$Z$};
    ...
    \path[->, line width= 1] 
        ...
        (x) edge (d)
        (x) edge (y);
    \path[->, line width= 1, purple]
        (z) edge [out=135, in=260, looseness=0.5] node[left, black] {$\text{Good variation in D}$} (d)
        (z) edge [out=45, in=280, looseness=0.5] (y);
\end{tikzpicture}
\end{center}
\end{figure}
